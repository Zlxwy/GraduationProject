...
if __name__ == "__main__":
  ...
  gVar.logger.PrintString("Thread_Recv has created.") # 打印 Thread_Recv 已创建信息
  cam_thread = threading.Thread(target=CamThreadFunc) # 相机线程，非守护线程，通过exit_flag通知退出
  gVar.logger.PrintString("Thread_Cam has created.") # 打印 Thread_Cam 已创建信息

  recv_thread.start() # 启动接收线程
  gVar.logger.PrintString("Thread_Recv has started.") # 打印 Thread_Recv 已启动信息
  cam_thread.start() # 启动相机线程
  gVar.logger.PrintString("Thread_Cam has started.") # 打印 Thread_Cam 已启动信息
