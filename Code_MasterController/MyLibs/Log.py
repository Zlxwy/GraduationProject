...
class Log:
  ...
  """
  def PrintString(self, msg: str, enabled=True):
    if not enabled: return
    self.log_queue.put(msg)

  def PrintHexBytes(self, msg: bytes, prefix="", suffix="", enabled=True):
    if not enabled: return
    hex_str = ""
    for byte in msg:
...
if __name__ == "__main__":
  logger = Log()
  logger.PrintString("Log has Initialized.")
  try:
    while True:
      time.sleep(1)
      logger.PrintString("Log is Working.")
  except KeyboardInterrupt:
    logger.PrintString("Program has been stopped by user.")
  finally:
    time.sleep(0.5) # 等待日志线程处理完成
    logger.stop() # 手动停止日志线程
