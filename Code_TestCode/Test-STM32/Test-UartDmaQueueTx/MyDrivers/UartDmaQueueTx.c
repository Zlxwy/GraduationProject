...
void UartDmaQueueTx_Init(UartDmaQueueTx_t *cThis,
  ...
  cThis->huart = huart;
  cThis->hdmatx = hdmatx;
  memset(cThis->TranBuf, 0, UART_TX_BUF_SIZE);
  memset(cThis->TxDataQueue, 0, sizeof(UartDmaQueueTx_Queue_t)*UART_TX_QUEUE_SIZE);
...
void UartDmaQueueTx_FuncCalled_InInfiniteLoop(UartDmaQueueTx_t* cThis) {
  if (cThis->TxDataQueue[cThis->HandleQueueIndex].DataLength != 0 && cThis->IsTxDmaIdle) { // 有数据待发送且DMA空闲
    ...
    //   (uint32_t)&cThis->TranBuf[ cThis->TxDataQueue[cThis->HandleQueueIndex].StartIndex ], // 传输缓冲区指针
...
void UartDmaQueueTx_SendArray(UartDmaQueueTx_t *cThis, const uint8_t *SendArray, size_t SendArrayLen) {
  ...
  if (StartIndexMark + SendArrayLen <= UART_TX_BUF_SIZE) {
    cThis->TxDataQueue[cThis->EnterQueueIndex].DataLength = SendArrayLen; // 记录数据长度
    memcpy( // 只需要一次复制
    ...
    cThis->TxDataQueue[cThis->EnterQueueIndex].StartIndex = StartIndexMark + SendArrayLen; // 更新下一条队列的传输缓冲区起始索引
    if (cThis->TxDataQueue[cThis->EnterQueueIndex].StartIndex >= UART_TX_BUF_SIZE) { // 如果下一条队列记录的传输缓冲区起始索引刚好满了
      // 其实进来这里的话，都不会在>UART_TX_BUF_SIZE的情况，只会是=UART_TX_BUF_SIZE的情况，因为上层if已经限制过了>UART_TX_BUF_SIZE的情况
      cThis->TxDataQueue[cThis->EnterQueueIndex].StartIndex -= UART_TX_BUF_SIZE; // 传输缓冲区索引从头开始
    }
  }
  
  /*数据超出传输缓冲区范围，分两次复制进传输缓冲区*/
  else if (StartIndexMark + SendArrayLen > UART_TX_BUF_SIZE) {
    cThis->TxDataQueue[cThis->EnterQueueIndex].DataLength = UART_TX_BUF_SIZE - StartIndexMark; // 记录数据长度
    memcpy( // 复制第一次
      &cThis->TranBuf[ StartIndexMark ],
      &SendArray[0],
      cThis->TxDataQueue[cThis->EnterQueueIndex].DataLength
    ...
    cThis->TxDataQueue[cThis->EnterQueueIndex].DataLength = SendArrayLen - (UART_TX_BUF_SIZE - StartIndexMark); // 记录数据长度
    memcpy( // 复制第二次
      &cThis->TranBuf[ 0 ],
