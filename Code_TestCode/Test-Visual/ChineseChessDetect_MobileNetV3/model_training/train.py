"""
中国象棋棋子分类 MobileNetV3 训练脚本

数据来源: data_collector.py 采集的 dataset/ 目录（7 类，128x128 JPG）
输出文件: chess_mobilenetv3.pt（模型权重）, train_history.json（训练记录）, train_curve.png（训练曲线）
...
BATCH_SIZE = 64       # RTX 4070 12GB 显存,增大 batch size 提高训练效率
EPOCHS = 1000         # 训练 1000 轮
LR = 1e-3             # ResNet-18 深层网络用小学习率更稳定
WEIGHT_DECAY = 1e-4   # 权重衰减防止过拟合
PATIENCE = 15         # 深层网络收敛稍慢,增加早停耐心值
TRAIN_RATIO = 0.8
SEED = 42

MODEL_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'chess_mobilenetv3.pt')
