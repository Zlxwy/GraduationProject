...
基于摄像头画面流，使用霍夫圆检测定位棋子，再用 MobileNetV3 模型分类。
依赖: chess_mobilenetv3.pt (train.py 训练产出)
...
MODEL_PATH = os.path.join(BASE_DIR, 'chess_mobilenetv3.pt')
...
def load_model():
    ...
    model = ChessMobileNetV3(num_classes=num_classes).to(DEVICE)
    ...
    while True:
        ...
        if circles is not None:
            ...
            for circle in circles[0, :]:
                ...
                pad = int(r * 1.2)
                ih, iw = frame_roi_copy.shape[:2]
                x1 = max(cx - pad, 0)
                y1 = max(cy - pad, 0)
                x2 = min(cx + pad, iw)
                y2 = min(cy + pad, ih)
                roi = frame_roi_copy[y1:y2, x1:x2]

                if roi.size == 0:
                    continue

                # 推理
                cls_idx, conf = predict(model, roi) # 通过模型推理，获取类别索引和置信度
                class_name = classes[cls_idx] # 通过类别索引，获取类别名称
                label_zh = CLASS_LABELS.get(class_name, class_name) # 从类别对应显示标签中获取中文标签
