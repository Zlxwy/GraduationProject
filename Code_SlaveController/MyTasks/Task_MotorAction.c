    ...
    if ( osMessageQueueGet(gMsgQueueMotorAction, &msg, NULL, 500) == osOK ) {
      if (msg.RequestType == MotorActionRequestType_BasicMove) { // 基础行走
        ...
        if (msg.ActionType == 0x00) { // 动作类型：0x00停止
          ...
          if (pStepperMotor != NULL) StepperMotor_Stop(pStepperMotor); // 步进电机停止
        }
          ...
          switch (msg.MotorID) { // 根据电机ID选择步进电机对象，进行操作
            case 0x00: // 肩关节步进电机
              while ( // 移动肩关节要等待肩关节和竖轴移动完成
                StepperMotor_GetRunState(&gStepperMotorShoulder) == StepperMotor_RunState_MoveSteps
                || StepperMotor_GetRunState(&gStepperMotorShoulder) == StepperMotor_RunState_RunContinuous
                || StepperMotor_GetRunState(&gStepperMotorLift) == StepperMotor_RunState_MoveSteps
                || StepperMotor_GetRunState(&gStepperMotorLift) == StepperMotor_RunState_RunContinuous
              ); pStepperMotor = &gStepperMotorShoulder;
              break;
            case 0x01: // 肘关节步进电机
              while ( // 移动肘关节要等待肘关节和竖轴移动完成
                StepperMotor_GetRunState(&gStepperMotorElbow) == StepperMotor_RunState_MoveSteps
                || StepperMotor_GetRunState(&gStepperMotorElbow) == StepperMotor_RunState_RunContinuous
                || StepperMotor_GetRunState(&gStepperMotorLift) == StepperMotor_RunState_MoveSteps
                || StepperMotor_GetRunState(&gStepperMotorLift) == StepperMotor_RunState_RunContinuous
              ); pStepperMotor = &gStepperMotorElbow;
              break;
            case 0x02: // 竖轴步进电机
              while ( // 移动竖轴要等待所有电机都移动完成
                StepperMotor_GetRunState(&gStepperMotorShoulder) == StepperMotor_RunState_MoveSteps
                || StepperMotor_GetRunState(&gStepperMotorShoulder) == StepperMotor_RunState_RunContinuous
                || StepperMotor_GetRunState(&gStepperMotorElbow) == StepperMotor_RunState_MoveSteps
                || StepperMotor_GetRunState(&gStepperMotorElbow) == StepperMotor_RunState_RunContinuous
                || StepperMotor_GetRunState(&gStepperMotorLift) == StepperMotor_RunState_MoveSteps
                || StepperMotor_GetRunState(&gStepperMotorLift) == StepperMotor_RunState_RunContinuous
              ); pStepperMotor = &gStepperMotorLift;
              break;
            default: break;
          }
          if (pStepperMotor != NULL) StepperMotor_MoveSteps(pStepperMotor, msg.Steps, msg.Speed); // 步进电机移动指定步数函数
          ...
          if (pStepperMotor != NULL) StepperMotor_RunContinuous( // 步进电机持续旋转函数
        ...
        if ( DigitalInput_IsTriggered(&gLimitSwitchVerticalAxis) ) { // 如果竖轴限位开关此时被触发着
          StepperMotor_MoveSteps(&gStepperMotorLift, 200, 1000); // 竖轴步进电机先往离开限位器方向转200步
          while (StepperMotor_GetRunState(&gStepperMotorLift) == StepperMotor_RunState_MoveSteps); // 等待步进电机移动指定步数完成
        }
        IsLimitSwitchVerticalAxisExtiEnabled = true; // 竖轴限位开关中断使能标志位
        StepperMotor_RunContinuous(&gStepperMotorLift, StepperMotor_Direction_Backward, 1000); // 竖轴步进电机持续往限位器方向转
        while (StepperMotor_GetRunState(&gStepperMotorLift) == StepperMotor_RunState_RunContinuous); // 等待步进电机持续移动完成
        ...
        int32_t steps = 1230.0f * msg.PositionZ / 100.0f; // 计算出要下降的步数
        StepperMotor_MoveSteps(&gStepperMotorLift, steps, 1000); // 竖轴步进电机移动指定步数函数
